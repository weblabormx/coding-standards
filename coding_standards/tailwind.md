...
**Use Tailwind utilities instead of direct CSS for static visual styling.**

Do not add static visual styling with direct CSS when Tailwind can express the same result. This applies whether the CSS is written in a `style=""` attribute, a `<style>` block, a CSS file, an imported stylesheet, or a custom class whose only purpose is static visual styling such as spacing, width, height, color, typography, borders, shadows, text decoration, or layout.

Report these findings as direct CSS styling and recommend replacing the CSS with Tailwind utilities. Do not turn the finding into a color-token recommendation.

Direct CSS is allowed when the value is genuinely dynamic at runtime, browser-specific, generated by a third-party integration, required by a vendor, defines reusable project infrastructure that Tailwind cannot express safely, or cannot be represented with Tailwind utilities. Examples include Alpine-bound progress widths, CSS variables populated from runtime data, vendor-required attributes, third-party widget styles, or chart/library container sizing that cannot be moved to Tailwind utilities.

When direct CSS is necessary, keep it as narrow as possible and do not mix unrelated static styling into it.
