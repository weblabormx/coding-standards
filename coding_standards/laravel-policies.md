...
- Prefer policies for reusable authorization and access rules tied to protected models or resources. Do not create ad hoc component helpers such as `canAccessVisitor()` or `canManageResource()` to hold authorization logic when there is a useful policy subject; put the rule in the policy and call it through Laravel authorization (`authorize`, `can`, `@can`, or equivalent project helpers).
- Inline checks are acceptable for local UI state, public flows, or one-off guards with no useful policy subject, but protected resource access should default to policy-backed authorization.
